	...
	if ( ! class_exists( 'Dianxiaomi' ) ) {
		...
		final class Dianxiaomi {
			...
			public function admin_styles() {
				wp_enqueue_style( 'dianxiaomi_styles_chosen', plugins_url( basename( __DIR__ ) ) . '/assets/plugin/chosen/chosen.min.css' );
				wp_enqueue_style( 'dianxiaomi_styles', plugins_url( basename( __DIR__ ) ) . '/assets/css/admin.css' );
			}

			public function library_scripts() {
				wp_enqueue_script( 'dianxiaomi_styles_chosen_jquery', plugins_url( basename( __DIR__ ) ) . '/assets/plugin/chosen/chosen.jquery.min.js' );
				wp_enqueue_script( 'dianxiaomi_styles_chosen_proto', plugins_url( basename( __DIR__ ) ) . '/assets/plugin/chosen/chosen.proto.min.js' );
				wp_enqueue_script( 'dianxiaomi_script_util', plugins_url( basename( __DIR__ ) ) . '/assets/js/util.js' );
				wp_enqueue_script( 'dianxiaomi_script_couriers', plugins_url( basename( __DIR__ ) ) . '/assets/js/couriers.js' );
				wp_enqueue_script( 'dianxiaomi_script_admin', plugins_url( basename( __DIR__ ) ) . '/assets/js/admin.js' );
			}
			public function include_footer_script() {
				wp_enqueue_script( 'dianxiaomi_script_footer', plugins_url( basename( __DIR__ ) ) . '/assets/js/footer.js', true );
			}
			...
			function display_tracking_info( $order_id, $for_email = false ) {
				...
				} elseif ( $this->plugin == 'wc-shipment-tracking' ) { // $49
			...
			private function display_order_dianxiaomi( $order_id, $for_email ) {
				...
				} elseif ( $dianxiaomi_tracking_provider_name == 'DHL' ) {
				...
				} elseif ( $dianxiaomi_tracking_provider_name == 'FedEx' ) {
				...
				} elseif ( $dianxiaomi_tracking_provider_name == 'UPS' ) {
